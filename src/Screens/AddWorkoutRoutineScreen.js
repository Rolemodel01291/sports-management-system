...
class AddWorkoutRoutineScreen extends React.Component{
    constructor(props){
        super(props)
        this.state={           
            pushups   :'',
    ...
    addGameTracker =async () => {
        ...
        firebase.database().ref(`members/${this.props.id}/routines/${monthNames[this.state.date.split('-')[1]-1]}/${this.state.date}`).set({
                    ...
                    <ImageBackground source={backgroundImg} style={{width:width, height:height/1.039}}>
                        ...
                        <View style={{flex:0.7, padding:10, paddingHorizontal:20}}>
                            ...
                            <TextInput style={{backgroundColor:'white',paddingHorizontal:20, marginTop:10, borderRadius:3,shadowColor: "#000",shadowOffset: {	width: 0,	height: 2,},shadowOpacity: 0.25,shadowRadius: 3.84,elevation: 5,}}
                                ...
                                onChangeText={(pushups)=>this.setState({pushups})}>
                            </TextInput>
                            <TextInput style={{backgroundColor:'white',paddingHorizontal:20, marginTop:10, borderRadius:3,shadowColor: "#000",shadowOffset: {	width: 0,	height: 2,},shadowOpacity: 0.25,shadowRadius: 3.84,elevation: 5,}}
                                placeholder="Enter Pull-ups"
                                value={this.state.pullups}
                                keyboardType={'numeric'}
                                onChangeText={(pullups)=>this.setState({pullups})}>
                            </TextInput>
                            <TextInput style={{backgroundColor:'white',paddingHorizontal:20, marginTop:10, borderRadius:3,shadowColor: "#000",shadowOffset: {	width: 0,	height: 2,},shadowOpacity: 0.25,shadowRadius: 3.84,elevation: 5,}}
                                placeholder="Enter Sit-ups"
                                value={this.state.situps}
                                keyboardType={'numeric'}
                                onChangeText={(situps)=>this.setState({situps})}>
                            </TextInput>
                            <TextInput style={{backgroundColor:'white',paddingHorizontal:20, marginTop:10, borderRadius:3,shadowColor: "#000",shadowOffset: {	width: 0,	height: 2,},shadowOpacity: 0.25,shadowRadius: 3.84,elevation: 5,}}
                                placeholder="Enter Run"
                                value={this.state.run}
                                keyboardType={'numeric'}
                                onChangeText={(run)=>this.setState({run})}>
                            </TextInput>
                                ...
                                <View style={{height:50, justifyContent:'center', alignItems:'center', borderRadius:3,backgroundColor:'#DFDFDF',shadowColor: "#000",shadowOffset: {	width: 0,	height: 2,},shadowOpacity: 0.25,shadowRadius: 3.84,elevation: 5}}>                                    
                                    <View style={{flexDirection:'row', justifyContent:'center', alignItems:'center'}}>
                                        <Icon 
                                            ...
                                            size={20}
                                            color="white"/>
                                        <Text style={{color:'white', fontSize:18, marginLeft:15}}>Add Routine</Text>
                                    </View>                           
                                </View>
                            </TouchableOpacity>
...
export default connect(mapStateToProps, mapDispatchToProps)(AddWorkoutRoutineScreen);
